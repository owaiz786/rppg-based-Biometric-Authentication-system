...
def generate_challenge_token(n_challenges: int = 2) -> Dict:
    ...
    Returns:
        {
          ...
          "imu_challenge_windows": List[Dict],   # timing windows for IMU verification
        }

    The frontend must display the challenges in this order, collect IMU data
    throughout, and send both the token and the IMU stream back with the video.
    The imu_challenge_windows field tells the backend which time slices of the
    IMU stream to examine for each challenge's expected motion signature.
    """
    import random
    ...
    # Build per-challenge timing windows.
    # FIXED_START = 2000 ms hold-still phase before first challenge.
    # Each challenge gets its own duration (from CHALLENGE_UI durations).
    CHALLENGE_DURATIONS_MS = {
        "blink":         3000,
        "head_turn":     3000,
        "head_left":     2500,
        "head_right":    2500,
    ...
    FIXED_START_MS = 2000  # hold_still phase
    cumulative_ms  = float(FIXED_START_MS)
    imu_windows    = []
    for cid in challenges:
        duration   = CHALLENGE_DURATIONS_MS.get(cid, 2500)
        imu_windows.append({
            "challenge_id": cid,
            "start_ms":     cumulative_ms,
            "end_ms":       cumulative_ms + duration,
        })
        cumulative_ms += duration

    _token_store[token] = {
        "challenges":            challenges,
        "imu_challenge_windows": imu_windows,
        "issued_at":             time.time(),
...
def get_imu_windows_for_token(token: str) -> List[Dict]:
    """
    Return the stored IMU challenge windows for a token WITHOUT consuming it.
    Called by main.py after consume_challenge_token has already validated and
    consumed the token, using the same entry before it is garbage-collected.

    Since the token is already marked used by consume_challenge_token, we
    look it up by allowing used=True here.
    """
    entry = _token_store.get(token)
    if entry is None:
        return []
    return entry.get("imu_challenge_windows", [])
